...
class M_DFRobot_HX711_I2C(object):
  ...
  def __init__(self ,SCL, SDA, address):
    ...
    self.i2cbus = machine.I2C(SCL, SDA)
    self._addr = address
    self.idle =    0
  ...
  def write_data(self, data):
    self.i2cbus.writeto(self._addr ,data)
    
  def write_reg(self, reg, data):
    self.i2cbus.writeto(self._addr ,reg)
    self.i2cbus.writeto(self._addr ,data)
  ...
  def read_reg(self, reg ,len):
    ...
    for i in range(len):
      #time.sleep(0.03)
      self.rxbuf[i] = self.i2cbus.readfrom(self._addr, 1)
    #print(self.rxbuf)
    return self.rxbuf